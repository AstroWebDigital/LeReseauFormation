...
export const DocumentModal = ({ isOpen, onOpenChange, formData, setFormData, onSave, isEdit, inputClasses, isDark }) => {
    ...
    // Classes Select adaptées au thème
    const selectClasses = {
        label: isDark ? "!text-white !opacity-100 font-bold text-sm" : "!text-slate-700 !opacity-100 font-bold text-sm",
        trigger: isDark
            ? "border-slate-700 bg-[#0f1129] hover:border-white transition-all min-h-[44px]"
        ...
        popoverContent: isDark
            ? "bg-[#0f1129] border border-slate-800 text-white"
            : "bg-white border border-slate-200 text-slate-800",
    };

    // Couleurs de la modale
    const modalBg = isDark ? "bg-[#0f1129] text-white border border-slate-800" : "bg-white text-slate-800 border border-slate-200";
    const headerClass = isDark ? "text-white" : "text-slate-800";
    const footerBorder = isDark ? "border-t border-slate-800" : "border-t border-slate-200";
    const cancelClass = isDark ? "text-white border-slate-700" : "text-slate-700 border-slate-300";
    ...
    return (
        <Modal isOpen={isOpen} onOpenChange={onOpenChange} size="2xl" className={modalBg}>
            <ModalContent>
                {(onClose) => (
                    <>
                        <ModalHeader className={`text-xl font-bold ${headerClass}`}>
                        ...
                        <ModalBody className={`grid grid-cols-1 md:grid-cols-2 gap-4 py-6 ${
                            ...
                            <div className="md:col-span-2">
                                ...
                                >
                                    ...
                                    {selectedFile ? (
                                        ...
                                        <>
                                            <CloudArrowUpIcon className={`h-10 w-10 transition-colors group-hover:text-orange-500
                            ...
                            <Select label="Domaine *" variant="bordered" labelPlacement="outside" classNames={selectClasses}
                            ...
                            <Select label="Type de document *" variant="bordered" labelPlacement="outside" classNames={selectClasses}
                            ...
                            <Select label="Statut *" variant="bordered" labelPlacement="outside" classNames={selectClasses}
                            ...
                            <Input label="Date d'émission *" type="date" variant="bordered" labelPlacement="outside"
                                   classNames={inputClasses} value={formData.issueDate || ""}
                            ...
                            <Input label="Date d'expiration" type="date" variant="bordered" labelPlacement="outside"
                                   classNames={inputClasses} value={formData.expirationDate || ""}
                                   onChange={(e) => setFormData({ ...formData, expirationDate: e.target.value })} />

                            <Select label="Sélectionner le véhicule" variant="bordered" labelPlacement="outside"
                        ...
                        <ModalFooter className={footerBorder}>
                            <Button variant="bordered" onPress={onClose} className={cancelClass}>Annuler</Button>
